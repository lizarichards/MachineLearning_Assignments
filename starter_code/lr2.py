...
def train_lr(data, eta, l2_reg_weight):
    ...
    for item in range(MAX_ITERS):
        # loss = 0
        weight_gradient = [0] * numvars
        bias_gradient = 0

        for sample, value in data:
            # x = sample = data[sample][0]
            # y = value = data[sample][1]
            # loss += log(sigmoid(-value * np.dot(w, sample) + b))
            error = sigmoid(value * (np.dot(w, sample) + b))
            bias_gradient -= value * error

            for k in range(numvars):
                # dw = ((sigmoid(-value * (dot_product + b))) * (value * x[k])) + (l2_reg_weight * w[k])
                # db = (sigmoid(-value * (dot_product + b))) * value
                ...
                weight_gradient[k] += -error * value * sample[k]

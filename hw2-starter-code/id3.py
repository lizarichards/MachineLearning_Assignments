...
#
# References: Worked with Tyler Christensen and Wil Sprouse
#
#
import sys
import re
...
def find_split_attr(data, vars):  # what arguments go in here?
    ...
    for i, var in enumerate(vars[:-1]):
        ...
        if temp_max_gain > max_gain:
            ...
            best_index = i
            best_attribute = var
    return max_gain, best_attribute, best_index # this index will be used in the splitting function
...
def collectValues(data, varnames, var):
    idx = varnames.index(var)
    values = []
    for entry in data:
        values.append(entry[idx])
    return values
...
def build_tree(data, varnames): # what exactly is varnames?
    ...
    last_item_0_count = 0
    last_item_1_count = 0
    max_gain, best_attribute, best_idx = find_split_attr(data, varnames)
    values = collectValues(data, varnames, best_attribute)
    ...
    elif last_item_1_count == sample_count:
    ...
    # case where a split can be done
        ...
        #varnames.remove(best_attribute)

...
def find_split_attr(data):  # what arguments go in here?
    max_gain = 0
    best_index = 0
    size = len(data) - 1
...
def build_tree(data, varnames): # what exactly is varnames?
    ...
    # first find the best attribute to split on
    # you need a loop here, keep splitting until you can no longer do that
    # turn the root into a leaf or a split, if it is not a leaf

    # 1. the leaf is pure: all sublist[-1] values are either 1 or 0 return a leaf with that value
    # 2. No attrubutes can be split on that yield information gain. Return a leaf with the dominant value from the -1 index
    # 3. The data can be split gainfully at an index (info gain > 0). Split it. Call build tree to create the left and right children/roots
    ...
    if last_item_0_count == sample_count:
        root = node.Leaf(varnames, 0)
    if last_item_1_count == sample_count:
        root = node.Leaf(varnames, 1)

    best_idx = find_split_attr(data)
    if best_idx == 0:
        ...
        left = build_tree(sublist1, varnames)
        right = build_tree(sublist2, varnames)
        root = node.Split(varnames, best_idx, left, right)
